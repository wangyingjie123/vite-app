...
import { batchDeleteUser, createUser, deleteUser, getUserById, getUserPage, updateUser } from '@/servers/system/user';
...
function Page() {
    ...
    return (
      <>
        ...
        {pagePermission.update === true && (
          <UpdateBtn className="mr-5px" isLoading={isLoading} onClick={() => onUpdate((record as RowData).id)} />
        )}
  ...
  const leftContentRender = (
    <>
      <DeleteBtn isIcon isLoading={isLoading} name={t('public.batchDelete')} handleDelete={handleBatchDelete} />
  ...
  return (
    <BaseContent isPermission={pagePermission.page}>
      {contextHolder}
      <BaseCard>
        <BaseSearch list={searchList(t)} data={searchData} type="grid" isLoading={isLoading} handleFinish={onSearch} />
      </BaseCard>
