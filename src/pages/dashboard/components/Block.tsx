...
function Block() {
  ...
  const data = [
    ...
    { title: t('dashboard.fileNumber'), num: 4286, all: 6142, icon: 'icon-park:file-display' },
    { title: t('dashboard.folderNumber'), num: 5649, all: 5232, icon: 'icon-park:file-cabinet' },
    { title: t('dashboard.uploadToday'), num: 619, all: 2132, icon: 'icon-park:file-addition' },
  ];

  return (
    <Row gutter={16}>
      {data.map((item) => (
        <Col key={item.title} sm={24} md={12} lg={6} className={`pb-10px ${isPhone ? 'w-full' : ''}`}>
          <div
            className={`
                border
                border-gray-200
