...
import { type TableProps, type CheckboxProps, Button, Popover, Divider, Checkbox, message } from 'antd';
...
function FilterButton(props: Props) {
  ...
  const content = () => {
    return (
      <div className="min-w-130px flex flex-col">
        <Checkbox className="!px-12px" indeterminate={indeterminate} onChange={onCheckAllChange} checked={checkAll}>
          {t('public.checkAll')}
        </Checkbox>
        <Checkbox.Group className="flex flex-col !px-12px" value={checkedList} onChange={onChangeCheckbox}>
          {list?.map((item) => (
            <div key={item.value}>
  ...
  return (
    ...
    >
      <div {...params} className={`${className} inline-block cursor-pointer`}>
